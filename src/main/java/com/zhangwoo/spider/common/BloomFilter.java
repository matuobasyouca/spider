...
public class BloomFilter<E> implements Serializable {
	...
	private AtomicInteger numberOfAddedElements; // number of elements
	// actually added to
	// the Bloom filter
	private int k;
	static Charset charset = Charset.forName("UTF-8"); // encoding used for
	// storing hash values
	// as strings

	static String hashName = "MD5"; // MD5 gives good enough accuracy in most
	// circumstances. Change to SHA1 if it's
	// needed
	static final MessageDigest digestFunction;
	static { // The digest method is reused between instances to provide
		// higher entropy.
		MessageDigest tmp;
		try {
	...
	public BloomFilter(int bitSetSize, int expectedNumberOfFilterElements) {
		...
		numberOfAddedElements = new AtomicInteger(0);
	}
	...
	public BloomFilter(int bitSetSize, int expectedNumberOfFilterElements,
		...
		this.numberOfAddedElements = new AtomicInteger(
	...
	public double getFalsePositiveProbability() {
		return getFalsePositiveProbability(numberOfAddedElements.get());
	}
	...
	public  void add(E element) {
		...
		numberOfAddedElements.incrementAndGet();
	}
	 ...
	 */
	public int count() {
		return this.numberOfAddedElements.get();
	}
